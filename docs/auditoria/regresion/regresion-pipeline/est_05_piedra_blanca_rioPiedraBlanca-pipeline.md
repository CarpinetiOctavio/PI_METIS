...
|----------|-------|
| n | 39 |
| Media [m³/s] | 44.46666666666666 |
| Mediana [m³/s] | 31.0 |
| Varianza (no sesgada) [m³/s]² | 2099.583859649123 |
| Varianza (sesgada) [m³/s]² | 2045.7483760683763 |
| Desvío [m³/s] | 45.821216261128676 |
| Asimetría Sesgada | 1.7668585108675363 |
| Asimetría No Sesgada (g) | 1.9113739651703574 |
| Curtosis Sesgada | 6.51652647140837 |
| Curtosis No Sesgada (k) | 7.636988971026416 |
| Coeficiente de Variación (CV) | 1.0304621348079914 |
| Sumatoria ln(xi) | 122.48743076760032 |
| beta_0 = M0 | 44.46666666666667 |
| beta_1 = M1 | 33.9251012145749 |
| beta_2 = M2 | 27.852872305503887 |
| beta_3 = M3 | 23.87092163013216 |
| Máximo [m³/s] | 215.0 |
| Mínimo [m³/s] | 0.9 |
| Rango [m³/s] | 214.1 |
...
|-----------|-------|
| Estadístico (S-C) | 0.0 |
| Valor crítico (± lim) | 6.164414002968976 |
| Veredicto | aprobada |
| Warning | None |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico t | 1.8171690375904948 |
| n1 | 19 |
| n2 | 20 |
| Valor crítico (tabla) | 2.0261924630291093 |
| Veredicto | aprobada |

#### Cramer
| Parámetro | Valor |
|-----------|-------|
| Estadístico (t_w1) | 1.3214851128216802 |
| n_w1 | 24 |
| n_w2 | 12 |
| Valor crítico (tabla) | 2.0261924630291093 |
| Veredicto | aprobada |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (máx r_k) | 0.22968299705394135 |
| Valor crítico | 0.2874209338393221 |
| Veredicto | aprobada |

#### Wald-Wolfowitz
| Parámetro | Valor |
|-----------|-------|
| Estadístico Z | 0.37155739150293116 |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (Z tipificado) | 0.8707265026259213 |
| Valor crítico | 1.358 |
| Veredicto | aprobada |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (máx Z_i) | 2.3524213728829753 |
| Valor crítico (K_N) | 2.8571046816486043 |
| Veredicto | aprobada |
...
|---|---|---|---|---|---|---|
| Uniforme | Momentos | alfa = -34.8980 | beta = 123.8313 |  | 23.2826 | ok |
| Uniforme | Máxima Verosimilitud | alfa = 0.900000 | beta = 215.0000 |  | 72.2232 | ok |
| Exponencial beta | Momentos | beta = 0.022489 |  |  | 9.4244 | ok |
| Exponencial beta | Máxima Verosimilitud | beta = 0.022489 |  |  | 9.4244 | ok |
| Exponencial x0 y beta | Momentos | x0 = -1.3545 | beta = 45.8212 |  | 8.7670 | ok |
| Exponencial x0 y beta | Máxima Verosimilitud | x0 = -0.246491 | beta = 44.7132 |  | 9.4018 | ok |
| Generalizada Exponencial | Momentos | alfa = 0.934719 | lambda = 0.021522 |  | 8.7103 | ok |
| Generalizada Exponencial | Máxima Verosimilitud | alfa = 0.887956 | lambda = 0.020810 |  | 8.1182 | ok |
| Generalizada Exponencial | Momentos L | alfa = 0.237654 | lambda = -0.018431 |  | 96.2218 | ok |
| Normal | Momentos | mu = 44.4667 | sigma = 45.8212 |  | 20.2873 | ok |
| Normal | Máxima Verosimilitud | mu = 44.4667 | sigma = 45.8212 |  | 20.2873 | ok |
| Normal | Momentos L | mu = 44.4667 | sigma = 41.4356 |  | 20.4142 | ok |
| Log Normal (2 parámetros) | Momentos | mu_y = 3.1407 | sigma_y = 1.3799 |  | 27.1902 | ok |
| Log Normal (2 parámetros) | Máxima Verosimilitud | mu_y = 3.1407 | sigma_y = 1.3799 |  | 27.1902 | ok |
| Log Normal (3 parámetros) | Momentos | x0 = -35.3524 | mu_y = 4.2373 | sigma_y = 0.533704 | 10.4932 | ok |
| Log Normal (3 parámetros) | Máxima Verosimilitud | x0 = -2.1576 | mu_y = 3.3327 | sigma_y = 1.1133 | 8.7739 | ok |
| Gamma (2 parámetros) | Momentos | alfa = 47.2170 | beta = 0.941751 |  | 8.9581 | ok |
| Gamma (2 parámetros) | Máxima Verosimilitud | alfa = 49.1217 | beta = 0.905236 |  | 8.4314 | ok |
| Gamma (2 parámetros) | Momentos L | alfa = 50.7864 | beta = 0.875562 |  | 8.0077 | ok |
| Gamma (3 parámetros) | Momentos | x0 = -3.4792 | alfa = 43.7907 | beta = 1.0949 | 9.2217 | ok |
| Gamma (3 parámetros) | Máxima Verosimilitud |  |  |  | — | no_converge |
| Gumbel | Momentos | mu = 23.8471 | alfa = 35.7405 |  | 12.7527 | ok |
| Gumbel | Máxima Verosimilitud | mu = 25.9320 | alfa = 28.3984 |  | 17.2072 | ok |
| Gumbel | Momentos L | mu = 24.9941 | alfa = 33.7353 |  | 13.4882 | ok |
| Gumbel | Máxima Entropía | mu = 26.7138 | alfa = 30.7561 |  | 15.1952 | ok |
| GVE (Valores Extremos) | Momentos | nu = 695.2446 | alfa = 30.8225 | beta = -0.097895 | 697.5427 | ok |
| GVE (Valores Extremos) | Máxima Verosimilitud | nu = 19.5791 | alfa = 21.3856 | beta = -0.477719 | 6.3250 | ok |
| GVE (Valores Extremos) | Momentos L | nu = 21.6878 | alfa = 25.0565 | beta = -0.254306 | 9.0127 | ok |
| Log Pearson tipo III | Momentos Método Directo |  |  |  | — | no_aplicable |
| Log Pearson tipo III | Momentos Método Indirecto | alfa = 0.557206 | beta = 6.1327 | y0 = -0.276476 | 64.6816 | ok |
| Log Pearson tipo III | Máxima Verosimilitud |  |  |  | — | no_converge |
| Generalizada de Pareto | Momentos | mu = -2.0519 | sigma = 47.2319 | epsilon = 0.015334 | 7.1794 | ok |
| Generalizada de Pareto | Máxima Verosimilitud |  |  |  | — | no_converge |
| Generalizada de Pareto | Mínimos Cuadrados |  |  |  | — | no_converge |
| Generalizada de Pareto | Momento Prob. Pesada | mu = -16.0104 | sigma = 729.4913 | epsilon = 4.1386 | 125,625,290.5122 | ok |

### Notas de cableado — Bloque 6/7 (solo señalamiento, sin investigar causa)

**¿Ceros en la serie?** No — mínimo observado = 0.9 (`tiene_ceros=False` pasado a `ejecutar_etapa2()`). La rama `STATUS_DISABLED_ZEROS` no se ejercita en ninguna de las 4 estaciones auditadas hasta ahora (est_02, est_03, est_05, est_06) — sigue sin verificar en el pipeline real.

**Cambios de status vs. est_02** (comparando categoría, no valor numérico):
- `logpearson3/mv`: est_02=`ok` -> est_05=`no_converge`

**GVE / MV — rama de inicialización:**
- Descartada (guard IV-202 falla con nu0=695.2446, alpha0=30.8225, beta0 derivado de g). Fallback activo: Momentos-L (ML) como condición inicial para MV.