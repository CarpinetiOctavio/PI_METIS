...
|----------|-------|
| n | 24 |
| Media [m³/s] | 142.58333333333334 |
| Mediana [m³/s] | 105.0 |
| Varianza (no sesgada) [m³/s]² | 11818.949275362318 |
| Varianza (sesgada) [m³/s]² | 11326.493055555555 |
| Desvío [m³/s] | 108.71499103326238 |
| Asimetría Sesgada | 1.4657828696100466 |
| Asimetría No Sesgada (g) | 1.668559155919737 |
| Curtosis Sesgada | 4.594204926184641 |
| Curtosis No Sesgada (k) | 5.976876425708308 |
| Coeficiente de Variación (CV) | 0.7624663310339851 |
| Sumatoria ln(xi) | 113.24599225966217 |
| beta_0 = M0 | 142.58333333333334 |
| beta_1 = M1 | 99.7409420289855 |
| beta_2 = M2 | 79.40184453227931 |
| beta_3 = M3 | 66.86573028420854 |
| Máximo [m³/s] | 458.0 |
| Mínimo [m³/s] | 42.0 |
| Rango [m³/s] | 416.0 |
...
|-----------|-------|
| Estadístico (S-C) | 9.0 |
| Valor crítico (± lim) | 4.795831523312719 |
| Veredicto | rechazada |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico t | -1.7643073041726862 |
| n1 | 12 |
| n2 | 12 |
| Valor crítico (tabla) | 2.0738730679040147 |
| Veredicto | aprobada |

#### Cramer
| Parámetro | Valor |
|-----------|-------|
| Estadístico (t_w1) | 1.1397040134239187 |
| n_w1 | 15 |
| n_w2 | 7 |
| Valor crítico (tabla) | 2.0738730679040147 |
| Veredicto | aprobada |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (máx r_k) | 0.6302559806827854 |
| Valor crítico | 0.35621938964230415 |
| Veredicto | aprobada |

#### Wald-Wolfowitz
| Parámetro | Valor |
|-----------|-------|
| Estadístico Z | -2.0062402647738877 |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico | 1.6618950014661809 |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (Z tipificado) | 1.0206207261596576 |
| Valor crítico | 1.358 |
| Veredicto | aprobada |
...
| Parámetro | Valor |
|-----------|-------|
| Estadístico (máx Z_i) | 2.0146858452415364 |
| Valor crítico (K_N) | 2.6439099244338573 |
| Veredicto | aprobada |

**Veredicto general Etapa 1 (pipeline):** nivel_confianza=`con_warnings` — warnings: CONTRACT_LENGTH_WARNING, TEST_WARNING_SMALL_SAMPLE, TEST_WARNING_HOMOGENEITY
...
|---|---|---|---|---|---|---|
| Uniforme | Momentos | alfa = -45.7166 | beta = 330.8832 |  | 52.0788 | ok |
| Uniforme | Máxima Verosimilitud | alfa = 42.0000 | beta = 458.0000 |  | 125.1454 | ok |
| Exponencial beta | Momentos | beta = 0.007013 |  |  | 20.9118 | ok |
| Exponencial beta | Máxima Verosimilitud | beta = 0.007013 |  |  | 20.9118 | ok |
| Exponencial x0 y beta | Momentos | x0 = 33.8683 | beta = 108.7150 |  | 24.7462 | ok |
| Exponencial x0 y beta | Máxima Verosimilitud | x0 = 37.6268 | beta = 104.9565 |  | 27.0591 | ok |
| Generalizada Exponencial | Momentos | alfa = 1.8910 | lambda = 0.010212 |  | 26.4416 | ok |
| Generalizada Exponencial | Máxima Verosimilitud | alfa = 2.6281 | lambda = 0.012202 |  | 35.6104 | ok |
| Generalizada Exponencial | Momentos L | alfa = 0.343371 | lambda = -0.004898 |  | 325.1291 | ok |
| Normal | Momentos | mu = 142.5833 | sigma = 108.7150 |  | 47.0577 | ok |
| Normal | Máxima Verosimilitud | mu = 142.5833 | sigma = 108.7150 |  | 47.0577 | ok |
| Normal | Momentos L | mu = 142.5833 | sigma = 100.8242 |  | 47.6573 | ok |
| Log Normal (2 parámetros) | Momentos | mu_y = 4.7186 | sigma_y = 0.69901 |  | 27.6359 | ok |
| Log Normal (2 parámetros) | Máxima Verosimilitud | mu_y = 4.7186 | sigma_y = 0.69901 |  | 27.6359 | ok |
| Log Normal (3 parámetros) | Momentos | x0 = -69.9325 | mu_y = 5.2428 | sigma_y = 0.482137 | 29.6942 | ok |
| Log Normal (3 parámetros) | Máxima Verosimilitud | x0 = 38.4692 | mu_y = 4.0031 | sigma_y = 1.2927 | 29.6102 | ok |
| Gamma (2 parámetros) | Momentos | alfa = 82.8915 | beta = 1.7201 |  | 26.9310 | ok |
| Gamma (2 parámetros) | Máxima Verosimilitud | alfa = 64.0307 | beta = 2.2268 |  | 34.8765 | ok |
| Gamma (2 parámetros) | Momentos L | alfa = 82.2459 | beta = 1.7336 |  | 27.1573 | ok |
| Gamma (3 parámetros) | Momentos | x0 = 12.2733 | alfa = 90.6987 | beta = 1.4367 | 26.7550 | ok |
| Gamma (3 parámetros) | Máxima Verosimilitud |  |  |  | — | no_converge |
| Gumbel | Momentos | mu = 93.6616 | alfa = 84.7977 |  | 31.9309 | ok |
| Gumbel | Máxima Verosimilitud | mu = 98.0098 | alfa = 67.7597 |  | 43.3480 | ok |
| Gumbel | Momentos L | mu = 95.2013 | alfa = 82.0873 |  | 33.0922 | ok |
| Gumbel | Máxima Entropía | mu = 99.9833 | alfa = 73.8025 |  | 37.9837 | ok |
| GVE (Valores Extremos) | Momentos | nu = 2,328.9736 | alfa = 76.4126 | beta = -0.071531 | 2,385.2805 | ok |
| GVE (Valores Extremos) | Máxima Verosimilitud | nu = 77.6605 | alfa = 44.2612 | beta = -0.700168 | 37.6088 | ok |
| GVE (Valores Extremos) | Momentos L | nu = 86.5667 | alfa = 58.8357 | beta = -0.278289 | 28.2976 | ok |
| Log Pearson tipo III | Momentos Método Directo |  |  |  | — | no_aplicable |
| Log Pearson tipo III | Momentos Método Indirecto | alfa = 0.122115 | beta = 32.7664 | y0 = 0.717313 | 23.5367 | ok |
| Log Pearson tipo III | Máxima Verosimilitud | alfa = 0.84217 | beta = 1.1758 | y0 = 3.7284 | 83.5049 | ok |
| Generalizada de Pareto | Momentos | mu = 27.1338 | sigma = 122.8228 | epsilon = 0.063866 | 16.2521 | ok |
| Generalizada de Pareto | Máxima Verosimilitud |  |  |  | — | no_converge |
| Generalizada de Pareto | Mínimos Cuadrados |  |  |  | — | no_converge |
| Generalizada de Pareto | Momento Prob. Pesada | mu = -22.1572 | sigma = 1,808.5434 | epsilon = 4.1893 | 67,776,555.7978 | ok |

### Notas de cableado — Bloque 6/7 (solo señalamiento, sin investigar causa)

**¿Ceros en la serie?** No — mínimo observado = 42.0 (`tiene_ceros=False` pasado a `ejecutar_etapa2()`). La rama `STATUS_DISABLED_ZEROS` no se ejercita en ninguna de las 4 estaciones auditadas hasta ahora (est_02, est_03, est_05, est_06) — sigue sin verificar en el pipeline real.

**Cambios de status vs. est_02** (comparando categoría, no valor numérico):
- (fila de referencia — las comparaciones están en est_03/est_05/est_06)

**GVE / MV — rama de inicialización:**
- Descartada (guard IV-202 falla con nu0=2328.9736, alpha0=76.4126, beta0 derivado de g). Fallback activo: Momentos-L (ML) como condición inicial para MV.