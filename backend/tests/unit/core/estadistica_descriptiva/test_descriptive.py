...
# ── Rango — máximo - mínimo (tesis IV.5.1, RF-GEN-P-04) ──────────────────────


@pytest.mark.unit
def test_rango_es_maximo_menos_minimo(serie_facundo):
    arr = np.array(serie_facundo, dtype=float)
    rango_esperado = float(np.max(arr) - np.min(arr))

    resultado = calcular_descriptiva(serie_facundo)

    assert resultado.rango == pytest.approx(rango_esperado, abs=1e-10)
    assert resultado.rango == pytest.approx(
        resultado.maximo - resultado.minimo, abs=1e-10
    )
